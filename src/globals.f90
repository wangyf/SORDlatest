...
real, allocatable, target, dimension(:,:,:,:) :: &
    ...
    vv,             & ! velocity
    uu,             & ! displacement
    attr1,          & ! coarse-grain memory variables
    attr2,          &
    slv,		  & ! slip velocity only for input
    slvold,		  & ! slip velocity last step
    slu,		  & ! slip only for input
    sla,		  & ! slip acceleration only for input
    w1, w2,         &
    dep1, dep2,     &
    ep1, ep2,       &
    plmt1,plmt2,    &
...
real, allocatable, target, dimension(:,:,:) :: &
    ...
    trup,           & ! rupture time
    tarr,           & ! arrest time
    tn, ts,         & ! normal and shear traction magnitude
!    pore,           & ! pore pressure on fault
!    pore0,          & ! initial pore pressure on fault
    tnpc, tnold,    &
    mus, mud,       &
    dc,             &
    af, bf, v0, f0, &
...
real, allocatable, target, dimension(:,:,:,:) :: &
    ...
    tp, tptmp,      &
    t1, t2, t3,     &
    mort1, mort2  !moment rate tensor (normal, shear) 
    
! forced rupture with variable rupture velocity
integer :: &
	nstage, 	    & ! Number of stages of different rupture velocity (input)
	rupfrontstage     ! Segment index of current rupture front

real, allocatable, target, dimension(:) :: &
	vrupstage,		& ! rupture velocity in each segments (input)
	sizestage,		& ! size in each segments (input)
	tstage,			& ! starting rupture time in each segments
	rstage			  ! rupture radius in each segments

real :: &
	rrelax,			& ! prescribed cohesive zone size
	rupfront		  ! Analytical rupture distance (not discretized in mesh)

! attenuation (frequency-dependent)
real :: &
	fac, &		   ! factor to shift bandwith for constant Q only (f0 in paper)
 	q0,  &		   ! Q0 value in Q0*f^n relation (not directly used here)
	ex,  & 		   ! Exp of powerlaw (use 0.1:0.1:0.9 for Q(f),0.0 for constant
	fp			   ! reference frequency (velocity of input media)
