...
if ( attenuation == 'anelastic') then
	qs = 0.1 * mu
	qp = 2 * qs  !default assignation of qs and qp
	call fieldio( '<', 'qp',  qp  )
	call fieldio( '<', 'qs',  qs  )	
end if
...
if ( attenuation == 'anelastic') then
    call set_halo( qp,  0.0, i1, i2 )
    call set_halo( qs,  0.0, i1, i2 )
end if
...
if ( attenuation == 'anelastic') then
    call scalar_swap_halo( qp, nhalo )
    call scalar_swap_halo( qs, nhalo )
end if
...
! Implement dispersive wave speed and lame coefficient
if ( attenuation == 'anelastic') then
	call dispersion !modify s1 and s2
end if
...
! Output
call fieldio( '>', 'rho', mr  )
call fieldio( '>', 'vp',  s1  ) !note this vp and vs are already modified from attenuation
call fieldio( '>', 'vs',  s2  )
call fieldio( '>', 'gam', gam )
call fieldio( '>', 'mu',  mu  )
...
if ( attenuation == 'anelastic') then !note this qp and qs are already modified (Q0p-1 and Q0s-1)
	call fieldio( '>', 'qp',  qp  )
	call fieldio( '>', 'qs',  qs  )	
end if
end subroutine
