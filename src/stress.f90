...
if ( attenuation == 'anelastic') then
	call savestrain
    !if(debug>0 .and. master) write(0,'(E20.5)') maxval(abs(ss_tensor%strain2))
end if
...
! Attenuation
! save and update stress and memory variables
if ( attenuation == 'anelastic') then
	call savestress
	call updatememoryvariable
end if
...
do i = 1, 3
    z1(:,:,:,i) = w1(:,:,:,i) - s1 / pcnum
    if(pcnum == 2 .and. i == npln) z1(:,:,:,i) = w1(:,:,:,i)
    z2(:,:,:,i) = w2(:,:,:,i)
...
s2 = sqrt( 0.5 * s2 )
call invert( s2 )
r3 = - s1 / pcnum * sin( phi ) + mco * cos( phi )
r3 = r3 * s2
...
!output plastic moment tensor
	plmt1(:,:,:,i) = plmt1(:,:,:,i) + r1 !dmij = dsij 11,22,33
	plmt2(:,:,:,i) = plmt1(:,:,:,i) + r2 !23,31,12
...
call fieldio( '>', 'plmt11', plmt1(:,:,:,1) )
call fieldio( '>', 'plmt22', plmt1(:,:,:,2) )
call fieldio( '>', 'plmt33', plmt1(:,:,:,3) )
call fieldio( '>', 'plmt23', plmt2(:,:,:,1) )
call fieldio( '>', 'plmt31', plmt2(:,:,:,2) )
call fieldio( '>', 'plmt12', plmt2(:,:,:,3) )
